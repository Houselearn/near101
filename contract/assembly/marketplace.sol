...
contract RynoNFTMarket is Ownable, ReentrancyGuard {
    ...
    event ListingRemoved(address indexed collection, uint256 tokenId);
    ...
    function createListing(
        ...
        require(
            listingOf[_collection][_tokenId].owner == address(0),
            "Ryno:: LISTING_EXISTS"
        );
    ...
    function removeListing(address _collection, uint256 _tokenId)
        external
        nonReentrant
        returns (bool)
    {
    ...
    function buyListing(address _collection, uint256 _tokenId)
        external
        payable
        nonReentrant
        returns (bool)
    ...
    function setFeePerThousand(uint256 _fee) external onlyOwner returns (bool) {
        feePerThousand = _fee;
        return true;
    }

    function setMinPrice(uint256 _minPrice) external onlyOwner returns (bool) {
        minPrice = _minPrice;
        return true;
    }

    function setMaxPrice(uint256 _maxPrice) external onlyOwner returns (bool) {
        maxPrice = _maxPrice;
        return true;
    }
}
